...
with the `superpowers` plugin. It complements [`AGENTS.md`](AGENTS.md) (the
*why* of worktrees/specs) with the *how* of TDD and issue tracking.

Optimize for the **simplest approach that passes a test**, and **verify every
step with real output** before moving on.
...
```
brainstorm ──▶ spec ──▶ issue(s) ──▶ worktree ──▶ TDD ──▶ review ──▶ PR
  (skill)     (local)   (local)     (gitignored)  (R/G/R) (skill)
```

Everything in *local* / *gitignored* stays under `docs/superpowers/` and
`.worktrees/` — **never committed**. The durable record is the code and the PR.
...
- **Trivial** (one-line fix, typo, obvious tweak): skip straight to TDD on a
  branch. No spec, no issue.
- **Non-trivial** (new feature, behavior change, multi-file): run the full loop
  below, starting with brainstorm.

When unsure, treat it as non-trivial — a 10-line spec is cheap.

---

## 1. Brainstorm (non-trivial only)

Run `superpowers:brainstorm` to pressure-test the idea before any code. Goal:
agree on the **simplest** approach and surface unknowns. Output feeds the spec.

## 2. Write the spec — local only
...
## 3. Create issue(s) — local only

Break the spec into trackable units in the issue tracker (Linear-style):

```
docs/superpowers/issues/
```

See `docs/superpowers/issues/README.md` for the file format. One issue ≈ one
PR-sized chunk. Set `status: todo` and link the spec. Use issues to coordinate
**parallel agents** — each agent owns one issue in its own worktree.

## 4. Create a worktree

Implementation happens in an isolated worktree so the main checkout stays clean:

```bash
git worktree add -b feat/<scope>-<topic> .worktrees/<topic> main
```
...
`music-lyrics` and `thermal-qr` have no tests. If you change their behavior,
**add a test harness first** (the simplest one that fits):
...
(the `superpowers:verify` / `/verify` flow). Report what you observed.

## 7. Code review — always

Run `superpowers:requesting-code-review` on the branch before any PR.

- Report findings to the user **grouped by severity**.
- **Do not auto-fix.** The user decides scope. Re-review after agreed fixes.

## 8. PR

Open a PR only when no Important+ findings remain and the suite is green.
Conventional-commit title with scope (`feat(pdf_crop): …`). One feature per PR.
Mark the issue `done`.

---

## Parallel agents — the short version

For work that splits cleanly:

1. Spec it, then file N independent issues.
2. Create N worktrees (one per issue).
3. Dispatch one subagent per worktree in a **single message** so they run
   concurrently. Each agent gets: its issue, its worktree path, a "don't touch
   the main checkout or other worktrees" instruction, the code map, and a
   "report back briefly" instruction.
...
- **Local-only stays local.** `docs/superpowers/` and `.worktrees/` are
  gitignored. Never `git add` them.
- **Branch off `main`; never commit to `main`.** Commit/push only when asked.
