...
def resolve(src: Path, output: str | None = None, *, suffix: str = "_cropped") -> Path:
    ...
    `output` is None -> `<src.parent>/<src.stem><suffix>.pdf` (today's
    behavior, unchanged). Otherwise the `.pdf` suffix (case-insensitive) is the
    only signal: a `.pdf`-suffixed value is a **file target** (its parent dir +
    its exact name, no `<suffix>` appended); anything else is a **folder
    target** (the default `<src.stem><suffix>.pdf` written inside it). `~` is
    expanded and missing directories are created. `mkdir` failures (a parent
    that's a regular file, permission denied, ...) raise OutputPathError.
    """
    if output is None:
